# Objective
- Remove redundant saved indicators inside the Saved tab while keeping All/Public behavior unchanged.

# What changed (Saved tab only)
- Hidden the "נשמר" badge when viewing saved associations.
- Save toggle in Saved tab now reads "הסר" to indicate removal from saved list; All/Public tab unchanged.

# Files changed
- app/word/[wordId]/associations.tsx
- docs/codex/step-04a1-saved-tab-remove-label.md
...
## git diff --stat
```
 app/word/[wordId]/associations.tsx  | 474 ++++++++++++++++++++++--------------
...
+
   if (!wordId || !word) {
     return (
       <Screen withPadding>
@@ -123,58 +127,166 @@ export default function WordAssociationsScreen() {
     );
   }
...
@@ -207,98 +319,106 @@ const styles = StyleSheet.create({
   hebrew: {
     color: colors.muted,
   },
-  helper: {
...
- Add a public association, save it, go to Saved tab -> no "נשמר" label anywhere.
- In Saved tab the button reads "הסר" and removes it from Saved list.
- In All/Public tab the saved indicator still appears and saved button still works.
